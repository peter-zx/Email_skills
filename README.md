...
7. 支持生成“本轮全部PDF”平铺目录，方便一次性浏览本轮 PDF，优先使用硬链接，不重复占用 PDF 空间。
8. 对无法自动处理的邮件，会生成异常或人工介入任务，不静默跳过。
...
- 同时生成 `archive/本轮全部PDF/`，方便快速浏览本轮所有 PDF。Windows 上优先创建硬链接，看起来像真实 PDF，但不会额外复制一份 PDF 内容；如果硬链接失败，则创建 `.url` 指针文件作为兜底。
...
- `archive/本轮全部PDF/`：本轮所有 PDF 的平铺视图，优先使用硬链接，不额外复制 PDF 数据。
